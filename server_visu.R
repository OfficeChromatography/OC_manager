  ...
  tabsetPanel(
    tabPanel("Capture",
             column(3,
                    ...
                    column(6,
                           checkboxGroupInput("Visu_exp","Exposure choice",c(50,100,200,400,800),selected = 100)
                           ),
                    column(6,
                           checkboxGroupInput("Visu_ISO","ISO choice",c(100,200,400,800),selected = c(100))
                           ),
...
observeEvent(input$Visu_action,{
  ...
  # }else{
    withProgress(message = "Processing", value=0,min=0,max=17, {
      ...
      for(ISO in input$Visu_ISO){
        for(exp in input$Visu_exp){
          ...
          command = paste0("raspistill ","-o ",Visu_file," -n ","-ss ",exp,"000 -ISO ",ISO," -roi ",input$Visu_roi," -w 2000 -h 2000")
          if(input$Visu_awb_off){
            Visu_file = paste0("www/pictures/",input$Visu_name,"/",exp,"ms_iso_",ISO,"_awb_off.jpg")
            command = paste0("raspistill ","-o ",Visu_file," -n ","-ss ",exp,"000 -ISO ",ISO," -roi ",input$Visu_roi," -w 2000 -h 2000 -awb off")
          }
          system(command)
...
# observeEvent(input$Visu_delete_repo,{
#   if(input$Visu_name %in% dir("www/pictures/")){
#     system(paste0("rm -R /home/pi/OC_manager/www/pictures/",input$Visu_name))
#     Visu_feedback$text = "Directory deleted"
#     Visu_Dir$dir = dir("www/pictures/")
#   }
# })
...
observeEvent(input$Visu_RGB_254_LED,{
  # print((input$Visu_RGB_254_LED))
  # if(length(input$Visu_RGB_254_LED) == 0){
  #   python.call("send_cmd",paste0("M150 B0 U0 R0"))
  #   python.call("send_cmd",paste0("M42 P",42," S255"))
  # }else{
